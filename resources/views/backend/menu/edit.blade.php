...
<div id="edit-menu-page" class="container page menu-page">
	...
	<div class="main">
		<form id="edit-menu" action="{{route('creatMenu')}}" class="frm-menu" method="POST" name="create_menu" role="form"/>
		{!!csrf_field()!!}
		<div class="row">				
								...
								<li id="{{$count}}" class="ui-state-default item-{{$count}} old" data-position="{{$count}}">
									...
									<div class="row link-content">
										...
										<div class="dropdown show type-value col-md-5">
											@if($meta->type!="link")
												<a class="dropdown-toggle" href="#{{$meta->type}}" role="button" id="result-type-{{$count}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-value="{{$meta->value}}" data-meta="{{$meta->id}}">{{$meta->title_value}}</a>
			...
			<div class="col-md-4 sidebar right-box">			
				<section class="box-wrap">
					...
					<input type="text" name="alias" value="{{$menu->slug}}" class="frm-input mn-alias">
				</section>
			</div>				
		</div>
		<div class="group-action">
			<a href="{{route('deleteMenu',['id'=>$menu->id])}}" class="btn btn-delete">Xóa</a>
...
<script type="text/javascript">	
	$(function() {		
		...
		$(".frm-menu .mn-title").keyup(function(){
			var title = $(this).val();
			if(title.length>0)
				$(".frm-menu .mn-alias").val(convertToSlug(title));
			else
				$(".frm-menu .mn-title").val("");
		});
		//add recore		
       ...
       $(".menu-page").on('click','.frm-menu .group-action button',function(){
			...
			if(errors.length > 0){
	           ...
	           $.ajax({
	               	type:'POST',            
				    url:'{{ route("updateMenu",["id"=>$menu->id]) }}',
				    cache: false,
		            data:{
		                '_token': _token,					
						'title':title,
