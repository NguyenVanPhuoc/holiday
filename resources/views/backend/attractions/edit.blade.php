...
@php
	...
	$gallery = json_decode($attraction->gallery);
	$iconSelected = explode(",", $attraction->list_icon);
@endphp
<div id="edit-attraction" class="page route">
	<div class="head">
	...
	<div class="main">
		<form action="{{ route('updateAttractionAdmin', $attraction->slug) }}" method="post" class="dev-form edit-attraction activity-form">
			{!!csrf_field()!!}
			<div class="row">
				<div class="col-md-10 content">
					<div class="form-group" id="frm-title">
						<label for="title">Title<small>(*)</small></label>
						<input type="text" name="title" class="form-control" placeholder="Input title" value="{{$attraction->title}}" />
					</div>
					<div id="frm-desc" class="form-group">
						<label for="content">Description</label>
						<textarea name="desc" class="form-control" id="editor">{{$attraction->desc}}</textarea>
					</div>
					<div id="frm-gallery" class="form-group img-upload">
							...
							@if($gallery)
								@foreach($gallery as $value)
									...
									<div class="gallery-item item-{{$value}}" data-id="{{$value}}" >
										<div class="wrap-item">
											{!! imageAuto($value, $attraction->title) !!}
						...
						<div class="bot-wrap">
							<a href="{{ route('loadMedia') }}" class="btn btn-default library-gallery">Add to gallery</a><input type="hidden" name="gallery" class="thumb-media" value="{{$attraction->gallery}}">
						</div>
					</div>
					<div class="form-group">
						...
						<select multiple="multiple" name="list_icon[]" class="form-control select2">
                            @foreach ($list_icon as $item)
                                <option value="{{ $item->id }}" @if(in_array($item->id, $iconSelected)) selected @endif > {{ $item->title }} </option>
                            @endforeach
                        </select>
					</div>
				</div>
				<div class="col-md-2 sidebar">
					<div class="gr-not-fixed">
							...
							<select name="country_id" class="form-control select2">
								<option value="">Select city</option>
							                            @foreach ($list_highlight as $item)
							                                <option value="{{ $item->id }}" @if($item->id == $attraction->country_id) selected @endif> {{ $item->title }} </option>
							                            @endforeach
							                        </select>					
						</section> -->
						<section id="sb-country" class="box-wrap">
							...
							<select name="country_id">
								...
								@foreach($countries as $country)
									<option value="{{ $country->id }}" @if($country->id == $attraction->country_id) selected @endif>{{ $country->title }}</option>
								@endforeach
							</select>	
						</section>
						...
						<section id="sb-categories" class="box-wrap">
							...
							<div class="desc list">
								@php 
									$cat_active = explode(',',$attraction->cat_id);
								@endphp
	                            @if($categories)
						...
						<section id="sb-image" class="box-wrap">
							...
							<div id="frm-image" class="desc img-upload">							
								<div class="image">
									...
									{!!image($attraction->image, 150,150, $attraction->title)!!}
									<input type="hidden" name="image" class="thumb-media" value="{{$attraction->image}}" />
								</div>
							</div>
						</section>
						<section class="sb-time-info">
							<ul class="list-unstyled">
								<li><strong>Created at:</strong> {{ date('d M Y H:i', strtotime($attraction->created_at)) }}</li>
								<li><strong>Updated at:</strong> {{ date('d M Y H:i', strtotime($attraction->updated_at)) }}</li>
							</ul>
						</section>
					</div>
