...
<table  class="table table-striped">
	...
	<tbody>
		@foreach($list_consultant as $key => $c)
		<tr id="item-{{$c->id}}">
			<td class="check">
				<div class="checkbox checkbox-success">
			...
			<td class="avatar"><a href="{{ route('editConsultantAdmin', $c->id) }}">{!!image($c->image, 50,50, $c->title)!!}</a></td>
			<td class="title"><a href="{{ route('editConsultantAdmin', $c->id) }}">{{$c->title}}</a></td>
			<td class="tour-style">{{ $c->tourStyle->title }}</td>
			...
			<td class="action">
				...
				<a href="{{ route('editConsultantAdmin', $c->id) }}" class="edit"><i class="fa fa-edit" aria-hidden="true"></i></a>
				<a href="{{ route('deleteConsultantAdmin', $c->id) }}" class="delete delete-item"><i class="fa fa-close" aria-hidden="true"></i></a>					
			</td>
		</tr>
		@handheld
