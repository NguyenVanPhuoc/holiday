...
<div id="edit-guide" class="page route">
	<div class="head">
		<a href="{{route('guidesAdmin')}}" class="back-icon"><i class="fa fa-angle-left" aria-hidden="true"></i> All guides</a>
		<h1 class="title">Edit Guides</h1>		
	</div>
	<div class="main">
		<form action="{{ route('updateGuideAdmin', $guide->id) }}" method="post" class="dev-form activity-form">
			{!!csrf_field()!!}
			<div class="row">
...
<script type="text/javascript">
	...
	@endphp
	/*$(function() {
       $("#edit-guide").on('click','form .group-action button',function(){
	       	...
	       	if(error_count>0){
				...
				$.ajax({
					type:'POST',            
					url:'{{ route("updateGuideAdmin", ["slug"=>$guide->slug]) }}',
					cache: false,
					data:{
						'_token': _token,
						'longTitle': longTitle,
