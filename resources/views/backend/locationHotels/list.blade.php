...
@section('title','Location Hotel')
@section('content')
<div id="route-cats" class="container page routeCat-page">
	<div class="head">
		<h1 class="title">Location Hotel</h1>
		...
		<div class="col-md-8 right-box">
			<form id="form-routes" action="#" class="frm-menu" method="POST" name="route">
				...
				<section class="box-wrap">
					<h2 class="title">Location Hotel</h2>
					<ul class="sortable list-item">
						<?php $count = 0;?>
						@foreach($locations as $item) <?php $count++;?>						
							<li class="ui-state-default" data-value="{{$item->id}}" data-position="{{$count}}">{{$item->title}}
								<small>({{countTourInCat($item->id)}})</small>
								<a href="{{ route('editLocationHotelAdmin', ['slug'=>$item->slug]) }}">Edit<i class="fa fa-angle-down" aria-hidden="true"></i></a>
							</li>
						@endforeach
					</ul>
				</section>
	...
	$(function() {		
		$(".sortable" ).sortable({			
		    update: function(e, ui) {
				...
				$.ajax({
	               	type:'POST',            
				    url:'{{route("positionLocationHotelAdmin")}}',
				    cache: false,
		            data:{
		                '_token': _token,
						'routes': routes
