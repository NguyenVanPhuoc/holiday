...
<table class="table table-striped">
	...
	<tbody>
		...
		<tr id="item-{{$item->id}}">
			...
			<td class="title"><a href="{{ route('editMarketAdmin', $item->id) }}">{{$item->title}}</a></td>
			...
			<td class="action">
				...
				<a href="{{ route('editMarketAdmin', $item->id) }}" class="edit"><i class="fa fa-edit" aria-hidden="true"></i></a>
				<a href="{{ route('deleteMarketAdmin', $item->id) }}" class="delete delete-item"><i class="fa fa-close" aria-hidden="true"></i></a>					
			</td>
		</tr>
		@handheld
		<tr class="item-{{$item->id}} detail">
			<td colspan="7">					
				<ul class="info">
					<li>Long title: <a href="{{ route('editMarketAdmin', ['slug'=>$item->slug]) }}">{{$item->title}}</a></li>
					<li>
						Country: 
						@if(isset($countries))
