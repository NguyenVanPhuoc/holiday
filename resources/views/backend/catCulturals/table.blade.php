@php
/*
* table content of category cultural guide
*/
@endphp
		...
		@if ($list_catGuide->isNotEmpty())
			@foreach($list_catGuide as $key => $item)
				<tr id="item-{{ $item->id }}" data-id="{{ $item->id }}" data-position="{{ $key + 1 }}">
					...
					<td class="title"><a href="{{ route('editCatCulturalAdmin', $item->slug) }}">{{ $item->title }}</a></td>
					<td class="action">
						<a href="{{ route('editCatCulturalAdmin', $item->slug) }}" class="edit"><i class="fa fa-edit" aria-hidden="true"></i></a>
						<a href="{{ route('deleteCatCulturalAdmin', $item->id) }}" class="btn-delete delete"><i class="fa fa-close" aria-hidden="true"></i></a>
					</td>
				</tr>
			@endforeach
		@endif
