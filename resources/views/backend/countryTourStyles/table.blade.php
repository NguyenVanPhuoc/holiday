...
* @param request $list_country_tourStyle
*/
@endphp
...
<table class="table table-striped">
	...
	<tbody>
		@foreach($list_country_tourStyle as $key => $item)
		<tr id="item-{{$item->id}}">
			<td class="check">
				<div class="checkbox checkbox-success">
			...
			<td class="title"><a href="{{ route('editCountryTourStyleAdmin', $item->id) }}">{{$item->title}}</a></td>
			...
			<td class="action">
				...
				<a href="{{ route('editCountryTourStyleAdmin', $item->id) }}" class="edit"><i class="fa fa-edit" aria-hidden="true"></i></a>
				<a href="{{ route('deleteCountryTourStyleAdmin', $item->id) }}" class="delete delete-item"><i class="fa fa-close" aria-hidden="true"></i></a>					
			</td>
		</tr>
		@handheld
		<tr class="item-{{$item->id}} detail">
			<td colspan="7">					
				<ul class="info">
					<li>Long title: <a href="#">{{$item->title}}</a></li>
					<li>
