@desktop 
@if($list_tour_byCountry)
	<div class="tour_craft slide-style">
		@foreach($list_tour_byCountry as $item)
			@php
			...
			<div class="item">
				...
				<div class="desc light-graybg">
					...
					<a href="{{ route('tour', ['slug_country'=>$country_item->slug , 'slug'=>$item->slug]) }}">More</a>
				</div>
			</div>
		@endforeach
		@if(isset($list_tour_byCountry_v1))
			@foreach($list_tour_byCountry_v1 as $item)
				@php
					$country_item = get_country_of_tour($item->id)[0];
					$tourstyle_item = get_category_tour($item->cat_id);
					$countCountry = countDestinationsOfTour($item->id);
