@php
	$meta_key = $region->title . ' accommodation';
	$meta_value = 'The collection of ' . $region->title . ' accommodation including hotels, guesthouse, B&Bs, Lodges, or homestay. Filter for your favorite location or facilities of your stay';
	$title_tag = $region->title . ' accommodation: hotels, guesthouses, B&Bs, Lodges, or homestay';	

	$bg_img = getImgUrl($region->image, $region->title); 
	$icon_top = asset('public/images/icons/all/hotel2-white.png');
	$breadcrumb = Breadcrumbs::render('regionHotel', $country, $region->title);
	$title_top_h1 = $region->title . ' accommodation';
...
<div class="country-hotel hotels">
	...
	<div class="container">
		<p class="top-intro text-center font-semibold">
			Using our 10 years experence in {{ $country->title }}, discover our preferred hotels, resorts and lodges list that ensure you to receive an excellent service in only the finest surroundings
		</p>
	</div>
	<div class="section">
		<div class="container">
			<div class="hotel-content wrap-result">
				{!!csrf_field()!!}
				<input type="hidden" name="current_country" value="{{ $region->id }}">
